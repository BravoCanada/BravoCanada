								...
								<div class="form-area">
									<?php foreach($location as $rows):?>
										...
										<?php 
											...
											$this->db->where('course.course_tag',$course_tag);
										...
										<div class="form-check">
										  <label class="form-check-label" for="">
											<input type="radio" class="form-check-input location_radio" name="location_radio" value="<?php echo $rows->location_id;?>" <?php echo ($rows->location_id == $courseDetail->location_select) ? 'checked' :'';?>><?php echo $rows->location_name;?> (<?php echo $institute_count;?>)
										  </label>
										</div>
									<?php endforeach;?>									
							  </div>							  
								 ...
								 <input type="hidden" value="<?php echo ($course_tag) ? $course_tag : '';?>" name="hidden_tag_id" id="hidden_tag_id">															 							 
							</div>
						</div>
					</div>
				</div>
									...
									<div class="retreats-mid px-2 pt-4">
										...
										<div class="col-md-6">
											<div class="discover-btn text-right pr-3">
												<a href="<?php echo base_url('course/viewInstitute/'.$courseDetail->location_select.'/'.$rows->inst_id.'/'.$courseDetail->course_select);?>" class="btn btn-danger">Discover Now &nbsp;&nbsp;<i class="fa fa-angle-right custom" ></i></a>
											</div>
										</div>
									</div>
									</div>										
